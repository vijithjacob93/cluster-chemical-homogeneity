...
class fits_file(object):
    ...
    def cluster_members_init(self,dwarfs_or_giants):
        ...
        dr_16 = fits_file('allStar-r12-l33.fits')
        # merge with distance file to get distances for all APOGEE stars
        distance_data = fits_file("APOGEE_stars_distances/cbj_spectroPhotom_allStar_dr16beta_order.fits")        
        
        # get distances
        for i in range(len(distance_data)):
            distance_data[i]['APOGEE_ID'] = distance_data[i]['APOGEE_ID'].strip()
        dr_16_data['D2_med'] = distance_data['D2_med']        
        
        from astropy import table
        
        # join the cluster members file with the DR 16 file
#         self.data = dr_16_data
        self.data = table.join(dr_16_data, self.data,keys=['APOGEE_ID'])#['APOGEE_ID','Cluster','no_sigmas_RV','no_sigmas_PM',\
                                                     #'dist_center']
...
class Cluster():
    def __init__(self,name):#="",glon=0.0,glat=0.0,radius=0.0):
        from functions import Complete_Clusters
